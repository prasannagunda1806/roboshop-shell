...
status_check() {
  ...
  schema_setup() {
    ...
    mongo --host mongodb-dev.devopsprasanna1.store </app/schema/${component}.js &>>${log_file}
    status_check $?
    ...
    print_head "Load Schema"
    mysql -h mysql-dev.devopsprasanna1.store -uroot -p${mysql_root_password} < /app/schema/shipping.sql &>>${log_file}
    status_check $?
  fi
 }
